...
..  only:: draft
    ...
    work under Windows, Linux and MacOS. The ``Solver`` class uses by default a ``WallTimer`` internally and you can query 
    this timer to time events from the creation of the solver.
...
..  only:: draft
    ...
    * ``static int64 GetTimeInMicroSeconds()``
    ...
    that captures the time in *microseconds*.
    
    To measure the time, we query the system time and add or subtract the queried times. We measure thus *wall times*.
...
..  only:: draft
    ...
    This timer starts couting at the creation of the solver and is never reset.
        
    ..  warning:: The ``Solver``\'s integrated timer is never reset!

    To query this timer (and thus ask for the time elapsed since the creation of the ``Solver``):
